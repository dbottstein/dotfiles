function cless -d "Configure less to colorize styled text using environment variables before executing a command that will use less"
    ...
    set -q colored_man_bold; and set -l bold (set_color -o $colored_man_bold); or set -l bold (set_color -o 0280A5)
    set -q colored_man_standout; and set -l standout (set_color -r $colored_man_standout); or set -l standout (set_color -r F0CB02)
    set -q colored_man_underline; and set -l underline (set_color -u $colored_man_underline); or set -l underline (set_color -u 5BA502)
